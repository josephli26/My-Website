        ...
        <div className="flex items-center gap-2 flex-wrap justify-center mt-1">
          ...
          {imageClipboard && onPasteImage && (
            <button
              ...
              title="Paste image currently copied in clipboard into this row"
            >
...
function CMSGallerySectionEditor({
  ...
  sec: { type: "grid" | "row"; label: string; images: string[]; rows?: { id?: string; images: string[]; singleImageColumns?: number }[] };
  sIdx: number;
  allSections?: { sIdx: number; label: string }[];
  allTransferTargets?: { secIdx: number; rowIdx: number; label: string }[];
  imageClipboard?: { imgUrl: string; mode: "copy" | "move"; sourceSecIdx: number; sourceRowIdx: number; sourceImgIdx: number } | null;
  onUpdateSec: (updated: { type: "grid" | "row"; label: string; images: string[]; rows?: { id?: string; images: string[]; singleImageColumns?: number }[] }) => void;
  onRemoveSec: () => void;
  ...
  const rows: { id?: string; images: string[]; singleImageColumns?: number }[] =
