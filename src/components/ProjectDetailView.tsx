...
export function ProjectDetailView({ projectId, onBack }: ProjectDetailViewProps) {
  ...
  const rawVideoUrl = (project?.videoUrl || "").trim();
  const hasHeroVideo = Boolean(rawVideoUrl);
  let embedUrl = "";
        ...
        <section className="px-6 md:px-12 xl:px-16 max-w-[1550px] mx-auto cms-section-gap">
          <div className="overflow-hidden rounded-xl border border-white/5 shadow-2xl bg-black aspect-video w-full relative">
            {isPlayingVideo && hasHeroVideo ? (
              ...
              <div
                onClick={() => {
                  if (hasHeroVideo && !isGifModeForUrl(heroCover)) {
                    setIsPlayingVideo(true);
                  }
                }}
                className={`w-full h-full relative overflow-hidden flex items-center justify-center bg-neutral-900 group ${
                  hasHeroVideo && !isGifModeForUrl(heroCover) ? "cursor-pointer" : ""
                }`}
              >
                <ImageFallback
                  src={heroCover}
                  alt={`${project.title} Cover Hero`}
                  gifMode={isGifModeForUrl(heroCover)}
                  className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"
                />

                {/* Show dark overlay and Play Icon only if there's a showreel video AND heroCover is NOT in gifMode */}
                {hasHeroVideo && !isGifModeForUrl(heroCover) && (
                  <>
              ...
              {sec.type === "row" || !sectionRows ? (
                ...
                <div className="flex flex-col gap-6 md:gap-8 w-full items-center">
                  {sectionRows.map((rowItem, rIdx) => {
                    ...
                    const cols = rowItem.singleImageColumns && rowItem.singleImageColumns >= 1 ? rowItem.singleImageColumns : 1;
                    const styleRowMaxWidth = cols > 1 ? { maxWidth: `${Math.round(10000 / cols) / 100}%` } : {};

                    return (
                      <div
                        key={rIdx}
                        style={styleRowMaxWidth}
                        className="flex flex-row flex-nowrap gap-4 md:gap-6 w-full items-center justify-center mx-auto"
                      >
                        {rowItem.images.map((imgSrc, imgIdx) => {
                          ...
                          return (
                            <button
                              ...
                              className="overflow-hidden bg-brand-card/40 rounded-lg border border-white/5 group cursor-pointer focus:outline-none flex-1 min-w-0 flex items-center justify-center p-1 sm:p-2"
                            >
                              <ImageFallback
                                src={imgSrc}
